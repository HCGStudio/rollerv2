...
const preRoll = (difficulty: Difficulty) => {
  ...
  if (difficulty === Difficulty.normal || difficulty === Difficulty.hard) {
    const harder = randomSelect(41, 49, 3);
    harder.sort((a, b) => b - a);
    result.push(harder);
    const remain = randomSelect(1, 40, (order.length - 1) * 3);
    remain.sort((a, b) => b - a);
  ...
  const harder = randomSelect(34, 49, 6);
  harder.sort((a, b) => b - a);
  ...
  const remain = randomSelect(1, 33, (order.length - 2) * 3);
  remain.sort((a, b) => b - a);
  while (remain.length > 0) {
    const chunk = remain.splice(0, 3).sort((a, b) => b - a);
    result.push(chunk);
...
export const MutatorSelector: React.FC<IMutatorSelectorProps> = (
  ...
  const rollResult = React.useMemo(() => preRoll(props.difficulty), []);
